...
- **Session isolation** — Each account uses its own `BITWARDENCLI_APPDATA_DIR`
- **On-demand unlock** — Vault operations auto-unlock using password env vars; sessions are never persisted to disk
- **Authentication methods** — Master password (env var), API key (`--apikey`)
...
```bash
bw-plugin login              # Login (password env var: BWP_PASSWORD, etc.)
...
**Session model:** `bw-plugin` never persists session keys to disk. After login, run `unlock` to get a `BW_SESSION`:

```bash
# Capture session for current shell
export BW_SESSION=$(bw-plugin unlock --raw)

# Or eval it directly
eval $(bw-plugin unlock --raw | sed 's/^/export BW_SESSION=/')
```

Vault operations (`search`, `inject`, `totp`, `export`) auto-unlock on-demand if the password env var is set.
...
| Account | Email | Server | Password Env |
|---------|-------|--------|-------------|
| personal | `misterme00@icloud.com` | `vault.bitwarden.com` | `BWP_PASSWORD` |
| work | `i@mrme0.store` | `nodewarden.hmmr.workers.dev` | `BWW_PASSWORD` |
| api | `i@mrme0.store` | `vault.bitwarden.com` | `BWA_PASSWORD` |
...
State (active account only) is stored in `~/.config/bw-plugin/state.json`. **Session keys are never persisted** — they are derived on-demand via `bw unlock` using the password env var.
